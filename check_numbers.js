...
const {Ca: Ca, Ci: Ci, Pa: Pa, Pi: Pi} = v.cells;

check('gap today equals the Ca cell', h.gap_today, Ca);

check('gap as published equals the Pa cell', h.gap_published, Pa);

check('vintage difference, all types = Ca - Pa', Ca - Pa, h.vintage_all, .0025);

check('vintage difference, inventional = Ci - Pi', Ci - Pi, h.vintage_inv, .0025);

check('published-style contrast = Ca - Pi', Ca - Pi, h.contrast, .0025);

check('Shapley vintage = mean of both orderings', (Ca - Pa + (Ci - Pi)) / 2, h.shapley_vintage, .0025);

check('Shapley code type = mean of both orderings', (Ca - Ci + (Pa - Pi)) / 2, h.shapley_type, .0025);

check('Shapley parts sum to the contrast', h.shapley_vintage + h.shapley_type, h.contrast, .0025);

check('vintage share of the contrast rounds to 91%', Math.round(100 * h.shapley_vintage / h.contrast), 91);

check('excess over the null = V - V0', v.null.all.V - v.null.all.V0, h.excess_all, .0025);

check('excess over the null, inventional', v.null.inv.V - v.null.inv.V0, h.excess_inv, .0025);
...
check('type component path 1 closes', v.components.V_all.estimate + v.components.T_P.estimate, h.contrast, .0025);

check('type component path 2 closes', v.components.V_inv.estimate + v.components.T_C.estimate, h.contrast, .0025);

[ 'all', 'inv' ].forEach(t => v.migration[t].forEach((row, i) => check(`migration ${t} row ${i} sums to 1`, row.reduce((a, b) => a + b, 0), 1, .001)));

check('top bin stays top bin, 38.5%', Math.round(v.migration.all[3][3] * 1e3) / 10, 38.5);

check('top bin falls to zero, 21.9%', Math.round(v.migration.all[3][0] * 1e3) / 10, 21.9);

check('bin shares are shares of applications', v.raw.Ca.share5, v.raw.Ca.n5 / h.n_apps, .001);

check('top and zero bins together are a part of the sample', v.raw.Ca.n5 + v.raw.Ca.n0 < h.n_apps, true);

check('share of change at grant recomputed', (h.new_pairs_grant - h.new_pairs_pub) / (h.new_pairs_today - h.new_pairs_pub), h.share_at_grant, .001);

check('share of change at grant, from record.json', (rec.grant_time.new_pairs[1] - rec.grant_time.new_pairs[0]) / (rec.grant_time.new_pairs[2] - rec.grant_time.new_pairs[0]), rec.grant_time.share_at_grant, .001);
...
check('K1 band is 10% of the base', h.k1_band, h.k1_base / 10, .002);
...
check('variants run from +.13', Math.round(Math.min(...k.variants.map(x => x.beta)) * 100) / 100, .13);
...
check('K1 label Kill', lab.K1, 'Kill');

check('K2b label Support', lab.K2b, 'Support');

check('K2c label Kill', lab.K2c, 'Kill');

check('K6r label Kill', lab.K6r, 'Kill');

check('K4 label Inconclusive', lab.K4, 'Inconclusive');

check('D0 label Support', lab.D0, 'Support');

check('K7b label Support', lab.K7b, 'Support');

check('K7p label Support', lab.K7p, 'Support');

check('K3a label Support', lab.K3a, 'Support');

check('K6r wrong sign', k.ledger.find(t => t.id === 'K6r').estimate < 0, true);

check('K7b closure .977', Math.round(k.ledger.find(t => t.id === 'K7b').estimate * 1e3) / 1e3, .977);
...
check('post-decision removal moves the gap by -.70', Math.round((k.counterfactual['C_all minus post-disposition codes'] - k.counterfactual.C_all) * 100) / 100, -.7);

check('random removal moves the gap by -.60', Math.round((k.counterfactual['C_all minus random codes (benchmark)'] - k.counterfactual.C_all) * 100) / 100, -.6);
...
check('K4 = excess Q5 minus Q1', k.exposure.Q5.excess - k.exposure.Q1.excess, k.ledger.find(t => t.id === 'K4').estimate, .0025);
...
check('verdict B', k.verdict, 'B');

check('verdict B in headline', h.verdict, 'B');
...
check('lowest estimate', Math.min(...est), h.spec_min, .0015);

check('highest estimate', Math.max(...est), h.spec_max, .0015);

check('median estimate', median(est), h.spec_median, .0015);
...
check('published practice +.31', Math.round(lit[0][C.estimate] * 100) / 100, .31);
...
mv.forks.forEach(f => {
  ...
  const by = {};
  mv.rows.forEach(r => {
    (by[r[C[f]]] = by[r[C[f]]] || {})[key(r)] = r[C.estimate];
  });
  const lv = Object.keys(by);
  let best = 0;
  for (let i = 0; i < lv.length; i++) for (let j = i + 1; j < lv.length; j++) {
    const d = [];
    Object.keys(by[lv[i]]).forEach(kk => {
      if (by[lv[j]][kk] !== undefined) d.push(Math.abs(by[lv[i]][kk] - by[lv[j]][kk]));
    });
    if (d.length) best = Math.max(best, median(d));
  }
  check(`leverage of ${f}`, best, mv.leverage[f], .0015);
});
...
check('provenance gap today matches headline', P['Grant gap, record today'], h.gap_today, 6e-4);

check('provenance excess matches headline', P['Excess over the null, all code types'], h.excess_all, 6e-4);

check('provenance K1 in pp', P['Post-decision code, granted minus never granted'] * 100, h.k1_beta, 6e-4);

check('provenance K2b in pp', P['Post-window revision per SD of forward citations'] * 100, h.k2b_beta, 6e-4);

check('provenance share at grant', P['Share of a granted record’s change present at grant'], h.share_at_grant, 6e-4);
...
const text = JSON.stringify([ h, v, mv, k, rec, prov, ver, rule ]) + fs.readFileSync(path.join(__dirname, 'index.html'), 'utf8') + fs.readdirSync(path.join(__dirname, 'js')).filter(f => f.endsWith('.js')).map(f => fs.readFileSync(path.join(__dirname, 'js', f), 'utf8')).join('') + fs.readdirSync(path.join(__dirname, 'js', 'views')).map(f => fs.readFileSync(path.join(__dirname, 'js', 'views', f), 'utf8')).join('');
