/* Canvas helpers shared by every chart: device-pixel sizing, the paper’s palette, axes drawn the
   same way everywhere, the manuscript’s number formats, hover readouts and button keys. */
(function (global) {
  'use strict';

  var P = {
    earlier: '#1b2a49', current: '#b8236b', neutral: '#333333', charcoal: '#3b3b3b',
    survives: '#1d6f7a', killed: '#9aa1ad', band: '#e3e3e3', event: '#c9a227',
    grid: '#e3e3e3', spine: '#9a9a9a', faint: '#f4f4f4', ink: '#16181d',
    tintEarlier: '#D4EBF2', tintCurrent: '#EAD1DC', tintSurvives: '#D9EAD3', tintNeutral: '#BFBFBF',
  };
  ...
  /* Percentage points in the manuscript’s style: +3.53, -.60, +.37 */
  function pp(x, d) {
    d = d === undefined ? 2 : d;
    var v = Number(x);
  ...
  /* A bare number without a leading zero below one: .977, 2.85 */
  function dec(x, d) {
    d = d === undefined ? 2 : d;
    var v = Number(x);
    if (!isFinite(v)) return 'n/a';
  ...
  function num(x) { return Math.round(Number(x)).toLocaleString('en-US'); }
  function ci(arr, d) { return arr ? '[' + pp(arr[0], d) + ', ' + pp(arr[1], d) + ']' : ''; }
  function pval(p) {
  ...
  function axisX(ctx, x, y0, y1, ticks, fmt, label, o) {
    ...
    ctx.beginPath(); ctx.moveTo(x.range[0], y1 + 0.5); ctx.lineTo(x.range[1], y1 + 0.5); ctx.stroke();
    ctx.fillStyle = P.neutral;
    ctx.textAlign = 'center';
    ctx.font = '11px ' + FONT;
    ticks.forEach(function (t) { ctx.fillText(fmt(t), x(t), y1 + 12); });
  ...
  function axisY(ctx, y, x0, x1, ticks, fmt, label, o) {
    ...
    ctx.textAlign = 'right';
    ctx.font = '11px ' + FONT;
    ticks.forEach(function (t) { ctx.fillText(fmt(t), x0 - 6, y(t)); });
    if (label) {
      ctx.save();
  ...
  /* colour interpolation for morphing marks */
  ...
  /* hover readouts: hit(x, y, box) returns html or null; the tip follows the pointer */
  var tipEl = null;
  function hover(canvas, hit) {
    ...
    function move(e) {
      ...
      var html = hit(x, y, r);
      if (!html) { tipEl.hidden = true; canvas.style.cursor = ''; return; }
    ...
    canvas.addEventListener('pointerleave', function () { tipEl.hidden = true; canvas.style.cursor = ''; });
  }

  /* a group of real buttons; mode 'one' keeps exactly one on, 'many' toggles each */
  function keys(container, items, opts) {
    opts = opts || {};
    container.innerHTML = '';
    var state = {};
    items.forEach(function (it) { state[it.id] = it.on !== false; });
    var btns = {};
    items.forEach(function (it) {
      ...
      b.addEventListener('click', function () {
        if (opts.mode === 'one') {
          items.forEach(function (o) { state[o.id] = o.id === it.id; });
        } else {
    ...
    paint();
    return { state: state, set: function (id, v) { state[id] = v; paint(); }, buttons: btns };
  }

  /* greedy word wrap at a pixel width, for canvas labels of more than a few words */
  ...
  function debounce(fn, ms) {
    var h = null;
    return function () { clearTimeout(h); h = setTimeout(fn, ms || 120); };
  }

  global.D = { P: P, SEQ: SEQ, FONT: FONT, SERIF: SERIF, pp: pp, dec: dec, pct: pct, num: num, ci: ci, pval: pval,
    fit: fit, scale: scale, niceTicks: niceTicks, text: text, axisX: axisX, axisY: axisY,
    vline: vline, hline: hline, dot: dot, square: square, tri: tri, band: band,
    mix: mix, rgba: rgba, hover: hover, keys: keys, wrap: wrap, debounce: debounce };
}(window));
