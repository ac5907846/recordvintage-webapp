/* Findings view: the hero is started by boot.js; this file builds the three proposition tiles and
   the verdict line from data/headline.json. */
(function (global) {
  'use strict';
    ...
    init: function (h) {
      ...
      tiles.innerHTML = h.propositions.map(function (p) {
        return '<div class="tile">' +
          '<div class="pid">' + p.id + ' <span>' + p.name + '</span></div>' +
          '<div class="claim">' + p.claim + '</div>' +
          '<div class="tests">' + p.tests.map(function (t) {
      ...
      document.getElementById('verdictline').textContent =
        'Verdict of the frozen rule: ' + h.verdict + ', partially supported. Labels are those the rule assigned mechanically; nothing here is identified causally.';
    },
  };
