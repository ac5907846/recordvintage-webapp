  ...
  var Multiverse = {
    ...
    init: function(data) {
      ...
      data.forks.forEach(function(f) {
        self.active[f] = {};
        data.levels[f].forEach(function(v) {
          self.active[f][v] = true;
        });
      });
      this.buildControls();
      D.keys(document.getElementById('mvcolour'), [ {
        id: 'vintage',
        label: 'colour by state of the record',
        on: true,
        tone: 'neutral'
      }, {
        id: 'none',
      ...
      D.hover(document.getElementById('mvcanvas'), function(x, y) {
        return self.hit(x, y);
      });
    ...
    matching: function() {
      ...
      return d.rows.filter(function(r) {
        return d.forks.every(function(f) {
          return self.active[f][d.levels[f][r[ix[f]]]];
        });
      }).map(function(r) {
        var o = {
          est: r[iEst],
    ...
    render: function() {
      ...
      var box = D.fit(canvas, Math.max(300, Math.min(440, Math.round(w0 * .5))));
      ...
      D.axisY(ctx, y, L, w - R, [ -10, -5, 0, 5, 10, 15 ], function(t) {
        return t === 0 ? '0' : D.pp(t, 0);
      }, 'estimated grant gap, pp');
      D.hline(ctx, y(0), L, w - R, P.spine, 1.2);
      D.text(ctx, 'specifications, lowest to highest estimate', (L + w - R) / 2, h - 14, {
        size: 11,
        align: 'center'
      });
      var r = Math.max(1.6, Math.min(3.2, (w - L - R) / rows.length * .42));
      rows.forEach(function(p, i) {
        var px = x(i + .5), py = y(p.est);
        var col = self.colour === 'vintage' ? p.vint === 'current' ? P.current : P.earlier : P.charcoal;
        ctx.strokeStyle = col;
        ctx.globalAlpha = .22;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(px, y(Math.max(-10.5, p.lo)));
        ctx.lineTo(px, y(Math.min(18, p.hi)));
        ctx.stroke();
        ctx.globalAlpha = 1;
        ...
        if (p.flag) {
          ...
          D.text(ctx, lab, px + (left ? -12 : 12), py + 18, {
            size: 11,
      ...
      if (this.colour === 'vintage') {
        D.dot(ctx, L + 8, T - 8, 3.5, P.current);
        D.text(ctx, 'record today', L + 15, T - 8, {
          size: 11,
          color: P.current,
          weight: '600'
        });
        D.dot(ctx, L + 118, T - 8, 3.5, P.earlier);
        D.text(ctx, 'record as published', L + 125, T - 8, {
          size: 11,
          color: P.earlier,
          weight: '600'
        });
    ...
    hit: function(x, y) {
      ...
      var parts = d.forks.map(function(f) {
        return '<b>' + d.names[f].toLowerCase() + '</b> ' + d.labels[f][p.levels[f]];
      });
      return '<span class="num">' + D.pp(p.est) + 'pp</span> [' + D.pp(p.lo) + ', ' + D.pp(p.hi) + ']<br>' + parts.join('<br>') + '<br>n ' + D.num(p.n) + ', comparison grant rate ' + D.pct(p.g) + (p.flag ? '<br>' + d.flags[String(p.flag)] : '');
    },
    renderLeverage: function() {
      var d = this.data;
      var canvas = document.getElementById('levcanvas');
      var order = d.forks.slice().sort(function(a, b) {
        return d.leverage[b] - d.leverage[a];
      });
      var rowH = 20;
      var box = D.fit(canvas, order.length * rowH + 46);
      var ctx = box.ctx, w = box.w;
      var L = Math.min(150, w * .3);
      var x = D.scale(0, 4.8, L, Math.min(w - 60, L + 360));
      D.text(ctx, 'leverage of each choice, pp', L, 8, {
        size: 11.5
      });
      order.forEach(function(f, i) {
        var y = 26 + (i + .5) * rowH, v = d.leverage[f];
        D.text(ctx, d.names[f].toLowerCase(), L - 10, y, {
          size: 11.5,
          align: 'right'
        });
        D.hline(ctx, y, x(0), x(v), P.spine, 1);
        D.dot(ctx, x(v), y, 4.5, P.neutral, '#fff', 1);
        D.text(ctx, D.dec(v, 1), x(v) + 9, y, {
          size: 11.5,
          weight: '600'
        });
      });
      var yb = 26 + order.length * rowH + 4;
      D.axisX(ctx, x, yb, yb, [ 0, 2, 4 ], function(t) {
        return String(t);
      }, null, {
        grid: false
      });
