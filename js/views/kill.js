/* Kill tests: the ledger of Table III as a forest, each test on its own scale, with the label the
   frozen rule assigned; below it the 21 variants of K1 against the equivalence region. */
(function (global) {
  'use strict';

  var D = global.D, P = D.P;

  var UNIT = { 'pp': 'pp', 'pp per SD': 'pp per SD', 'new pairs': 'new pairs per first filing', 'closure': 'share of the gap closed', '': '' };
  var TONE = { Support: P.survives, Kill: P.killed, Inconclusive: P.charcoal, Exploratory: P.charcoal, Pass: P.survives };

  var Kill = {
    data: null, rows: [], vrows: [], vsort: 'order',

    init: function (data) {
      this.data = data;
      var self = this;
      D.keys(document.getElementById('variantkeys'), [
        { id: 'order', label: 'file order', on: true, tone: 'neutral' },
        { id: 'estimate', label: 'by estimate', on: false, tone: 'neutral' },
      ], { mode: 'one', onChange: function (s) { self.vsort = s.order ? 'order' : 'estimate'; self.renderVariants(); } });
      this.renderLedger();
      this.renderVariants();
      global.addEventListener('resize', D.debounce(function () { self.renderLedger(); self.renderVariants(); }));
      D.hover(document.getElementById('ledgercanvas'), function (x, y) { return self.hitLedger(x, y); });
      D.hover(document.getElementById('variantcanvas'), function (x, y) { return self.hitVariant(x, y); });
      document.getElementById('ledgernote').textContent =
        'Each row on its own scale, zero marked; shading is the preregistered equivalence region where the test has one. 95% intervals except K1, 90%; K5a carries a standard error, R0 is a reproduction gate. Table III of the article. Verdict of the frozen rule: ' + data.verdict + ', partially supported.';
      document.getElementById('variantnote').textContent =
        'K1 asks whether a symbol whose validity date postdates the cohort horizon, so could not have been on the record at the decision, is more often found on granted records. 90% cluster-robust intervals on 35 fields; the equivalence region is ±10% of the base rate. The logit row has no interval. Figure 4(b) of the article.';
    },
    ...
    renderLedger: function () {
      ...
      var qx = narrow ? 8 : 150, qw = narrow ? w - 16 : Math.round(w * 0.33);
      ...
      d.ledger.forEach(function (t, i) {
        ...
        if (i % 2 === 0) { ctx.fillStyle = P.faint; ctx.fillRect(0, y0, w, rowH); }
        D.text(ctx, t.id, narrow ? 8 : 10, ytxt, { size: 12.5, weight: '600' });
        self.chip(ctx, t.label, narrow ? 50 : 60, ytxt);
        var lines = D.wrap(ctx, t.question, narrow ? w - 150 : qw, 11.5);
        if (narrow) {
          lines.forEach(function (ln, j) { D.text(ctx, ln, 140, ytxt + j * 13, { size: 11, color: P.charcoal }); });
        } else {
          lines.forEach(function (ln, j) { D.text(ctx, ln, qx, y + (j - (lines.length - 1) / 2) * 13, { size: 11.5, color: P.charcoal }); });
        }
        /* the row scale: centred on zero and the estimate, wide enough for interval and band */
        var span = 0;
        ...
        if (!narrow) D.text(ctx, est, w - 8, y, { size: 11.5, align: 'right' });
        else D.text(ctx, est, w - 8, y0 + 38, { size: 10.5, align: 'right' });
        self.rows.push({ y0: y0, y1: y0 + rowH, t: t });
      });
    },

    shortQ: function (q) { return q; },

    chip: function (ctx, label, x, y) {
      var fills = { Support: P.tintSurvives, Kill: P.tintNeutral, Inconclusive: P.tintEarlier, Exploratory: P.faint, Pass: P.tintSurvives };
      ctx.save();
      ctx.font = '10.5px ' + D.FONT;
      var tw = ctx.measureText(label).width + 14;
      ctx.fillStyle = fills[label] || P.faint;
      ctx.fillRect(x, y - 8, tw, 16);
      if (label === 'Exploratory') { ctx.strokeStyle = P.grid; ctx.strokeRect(x + 0.5, y - 7.5, tw, 16); }
      ctx.restore();
      D.text(ctx, label, x + 7, y, { size: 10.5, color: '#000' });
    },

    hitLedger: function (x, y) {
      var row = null;
      this.rows.forEach(function (r) { if (y >= r.y0 && y < r.y1) row = r; });
      if (!row) return null;
      var t = row.t;
    ...
    renderVariants: function () {
      ...
      if (this.vsort === 'estimate') rows.sort(function (a, b) { return a.beta - b.beta; });
      ...
      /* the equivalence region differs by outcome, so each row draws its own */
      this.vrows = [];
      rows.forEach(function (v, i) {
        var y = top + (i + 0.5) * rowH;
        if (v.band) D.band(ctx, x(-v.band), x(v.band), y - rowH / 2, y + rowH / 2, P.band, null);
        self.vrows.push({ y0: y - rowH / 2, y1: y + rowH / 2, v: v });
      });
      D.axisX(ctx, x, top, bottom, [-1.5, -1, -0.5, 0, 0.5, 1, 1.5], function (t) { return t === 0 ? '0' : D.pp(t, 1); }, 'granted minus never granted, percentage points', { grid: true });
      ...
      rows.forEach(function (v, i) {
        ...
        if (!narrow) D.text(ctx, v.label, L - 10, y, { size: 11.5, align: 'right' });
        else D.text(ctx, v.label, axL + 4, y - 7, { size: 10, color: P.charcoal });
        if (v.lo !== null) D.hline(ctx, y, x(v.lo), x(v.hi), col, 1.6);
        D.dot(ctx, x(v.beta), y, v.lo === null ? 3 : 4, col, '#fff', 1);
        D.text(ctx, D.pp(v.beta), w - 8, y, { size: 11, align: 'right' });
      });
      if (!narrow) D.text(ctx, 'equivalence region', x(-1.55), top + 10, { size: 10.5, color: P.charcoal });
      /* the two primary tests as reference, drawn above the variants in the note colour */
    },

    hitVariant: function (x, y) {
      var row = null;
      this.vrows.forEach(function (r) { if (y >= r.y0 && y < r.y1) row = r; });
      if (!row) return null;
      var v = row.v;
